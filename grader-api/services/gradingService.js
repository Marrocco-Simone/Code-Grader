...
const runGradingContainer = async (graderContainerName, randomKey) => {
  ...
  await run([
    "docker",
    "cp",
    `${graderContainerName}-image:/app/submission/exitcode.data`,
    `exitcode-${randomKey}.data`,
  ]);
  ...
  const exitCode = await Deno.readTextFile(`exitcode-${randomKey}.data`);

  await Deno.remove(`result-${randomKey}.data`);
  await Deno.remove(`exitcode-${randomKey}.data`);

  return {
    grader_feedback: result.trim(),
    correct: exitCode.trim() === "0",
  };
};
...
const grade = async (code, testCode) => {
  ...
  const { grader_feedback, correct } = await runGradingContainer(
    graderContainerName,
    randomKey
  );

  return { grader_feedback, correct };
};
